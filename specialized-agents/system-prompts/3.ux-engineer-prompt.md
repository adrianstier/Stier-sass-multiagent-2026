...
### 6. Live Accessibility Testing with Playwright (CRITICAL)

**You MUST use Playwright to verify accessibility and user flows.** Do not rely on theoretical analysis - test the actual implementation.

#### Accessibility Verification Workflow
```
1. browser_navigate to the live URL
2. browser_snapshot to get the full accessibility tree
3. browser_press_key("Tab") repeatedly to test keyboard navigation
4. Verify focus order is logical and visible
5. browser_click on all interactive elements
6. browser_console_messages(level="warning") to check for a11y warnings
```
...
browser_take_screenshot("focus-1.png")  # Capture focus state
browser_press_key("Tab")  # Next element
browser_press_key("Enter")  # Activate buttons/links
browser_press_key("Escape")  # Close modals/dialogs
browser_press_key("Space")  # Toggle checkboxes
```
...
# - ARIA landmarks present (main, nav, etc.)
```
...
browser_navigate("{url}")
browser_click on primary CTA
browser_type in form fields
browser_select_option in dropdowns
# Verify feedback for each action
browser_take_screenshot("flow-complete.png")
```

#### When to Use Playwright
- **For accessibility audits**: Always run browser_snapshot first
- **For user flows**: Walk through each step with real interactions
- **For responsive a11y**: Test touch targets on mobile viewports
- **For focus management**: Verify focus states are visible and logical
- **For error states**: Test form validation and error announcements

**Never approve a design without live accessibility verification.**
