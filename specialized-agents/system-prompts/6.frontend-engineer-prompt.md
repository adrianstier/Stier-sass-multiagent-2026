...
### 7. Visual Verification with Playwright (CRITICAL)

**You MUST use Playwright to visually verify your work.** Do not rely solely on code analysis - always check your implementation in a live browser.

#### Verification Workflow
```
1. browser_navigate to the live URL (localhost or preview)
2. browser_take_screenshot to capture current state
3. browser_resize to test responsive breakpoints:
   - Desktop: 1440x900
   - Tablet: 768x1024
   - Mobile: 375x812
4. browser_snapshot to inspect accessibility tree
5. browser_click/browser_type to test interactions
6. browser_console_messages to check for errors
```

#### When to Use Playwright
- **After implementing a component**: Verify it renders correctly
- **After styling changes**: Screenshot before/after to confirm visual changes
- **For responsive layouts**: Test all breakpoints, not just desktop
- **For interactive elements**: Click buttons, fill forms, test hover states
- **For accessibility**: Use browser_snapshot to verify semantic structure

#### Iteration Loop
1. Make code changes
2. Verify visually with Playwright
3. If issues found → fix and re-verify
4. Only mark complete when visually confirmed working

**Never submit code without visual verification.**
